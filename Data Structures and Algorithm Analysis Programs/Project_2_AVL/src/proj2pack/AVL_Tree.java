...
public class AVL_Tree {
	...
	public static class AVLTree {
		...
		AVLNode insert(AVLNode node, String key) {
			...
			if (balance > 1 && Integer.parseInt(key) < Integer.parseInt(node.left.key)) {
				System.out.println("Imbalance occurred at inserting ISBN " + key +
						"; fixed in right rotation");
				return rightRotate(node);
			}

			// Right Right Case
			if (balance < -1 && Integer.parseInt(key) > Integer.parseInt(node.right.key)) {
				System.out.println("Imbalance occurred at inserting ISBN " + key +
